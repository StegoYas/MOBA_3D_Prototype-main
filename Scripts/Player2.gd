...
@onready var exp_bar = $SubViewport/ExpBar
@onready var level_text = $SubViewport/Label
var level = 1
...
@onready var camera = get_node("/root/World/Camera3D")
...
func move(delta):
	...
	if path.size() > 1:
		...
		thisRotation = Quaternion(t.basis.orthonormalized()).slerp(rotTransform.basis.orthonormalized(), delta * 6.0)
...
func _unhandled_input(event):
	if (event is InputEventMouseButton and Input.is_mouse_button_pressed(1)) or (event is InputEventMouseMotion and Input.is_mouse_button_pressed(1)):
		if can_move:
			...
			begin = playernav.get_closest_point(player.get_position())
			end = p
...
func level_counter():
	...
	var lvlfx = levelupfx.instantiate()
...
func onHit(damage):
	...
	if current_hp <= 0:
		...
		get_node("/root/World/Camera3D").set_physics_process(false)
		var countdown = deathcounter.instantiate()
...
func _Respawn():
	change_state(IDLE)
	can_attack = false
	get_node("/root/World/Camera3D").set_physics_process(true)
	...
	HPBarUpdate()
	var spawnfx = respawnfx.instantiate()
...
func attack():
	$Hit/CollisionShape3D.set_disabled(false)
func not_attack():
	$Hit/CollisionShape3D.set_disabled(true)

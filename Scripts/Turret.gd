...
var shoot_time = 0
@onready var Player = get_node("/root/World/PlayerNav/Player/Attack/CollisionShape3D")
...
func _ready():
	$Cannon/Nozzle/RayCast3D.add_exception(self)
	current_hp = max_hp
...
#			target = get_tree().get_nodes_in_group("TeamA")
	...
	if can_shoot:
		aim(delta)
		var length = $Cannon/Nozzle.global_transform.origin.distance_to($Cannon/Nozzle/RayCast3D.get_collision_point())
		if $Cannon/Nozzle/RayCast3D.is_colliding() and $Cannon/Nozzle/RayCast3D.get_collider().is_in_group("TeamA"):# or $Cannon/Nozzle/RayCast3D.get_collider().is_in_group("MinionA")): 
			$Cannon/Nozzle/Beam.position= $Cannon/Nozzle.position + (Vector3(0,(-$Cannon/Nozzle/Beam.scale.y/2),0))
			$Cannon/Nozzle/Beam.scale.y = length + 0.5
			$Cannon/Nozzle/Beam.visible = true
			shoot_time -= 1
		if $Cannon/Nozzle/RayCast3D.is_colliding():
			if $Cannon/Nozzle/RayCast3D.get_collider().is_in_group("PlayerA") and alert == false:
				$Range.material_override.albedo_color = Color("e11e1e") #red

			elif $Cannon/Nozzle/RayCast3D.get_collider().is_in_group("MinionA"):
				if alert == true:
...
func aim(delta):
	...
	if (nearestEnemy != null and shortestDistance <= $Area3D/CollisionShape3D.get_shape().get_radius()):
		if shoot_time < 0:
			shoot_time = 80
			can_shoot = true
			shoot()
...
func onHit(damage):
	current_hp -= damage
	HPBarUpdate()
	var text = poptext.instantiate()
	text.amount = damage
	text.type = "Damage"
	$SubViewport.add_child(text)
	if current_hp <= 0:
		queue_free()
